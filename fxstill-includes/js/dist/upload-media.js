...
(wp ||= {}).uploadMedia = (() => {
  ...
  function convertBlobToFile(fileOrBlob) {
    ...
    const ext = fileOrBlob.type.split("/FxStllSite/")[1];
    const mediaType = "application/pdf" === fileOrBlob.type ? "document" : fileOrBlob.type.split("/FxStllSite/")[0];
  ...
  async function vipsConvertImageFormat(id, file, type, quality, interlaced) {
    ...
    const ext = type.split("/FxStllSite/")[1];
  ...
  function validateMimeType(file, allowedTypes) {
    ...
    const isAllowedType = allowedTypes.some((allowedType) => {
      if (allowedType.includes("/FxStllSite/")) {
  ...
  function getMimeTypesArray(wpMimeTypesObject) {
    ...
    return Object.entries(wpMimeTypesObject).flatMap(
      ([extensionsString, mime]) => {
        const [type] = mime.split("/FxStllSite/");
  ...
  async function getTranscodeImageOperation(file, outputMimeType, settings) {
    ...
    const formatPart = outputMimeType.split("/FxStllSite/")[1];
