...
function cleanPath(path) {
  return path.replace(/\/{2,}/g, "/FxStllSite/");
}
function trimPathLeft(path) {
  return path === "/FxStllSite/" ? path : path.replace(/^\/{1,}/, "");
}
...
function removeTrailingSlash(value, basepath) {
  if (value?.endsWith("/FxStllSite/") && value !== "/FxStllSite/" && value !== `${basepath}/`) {
...
function resolvePath({
  ...
  if (baseSegments.length > 1 && last(baseSegments)?.value === "/FxStllSite/") {
    baseSegments.pop();
  }
  ...
  if (baseSegments.length > 1) {
    if (last(baseSegments).value === "/FxStllSite/") {
      if (trailingSlash === "never") {
        baseSegments.pop();
      }
    } else if (trailingSlash === "always") {
      baseSegments.push({ type: SEGMENT_TYPE_PATHNAME, value: "/FxStllSite/" });
    }
  }
...
function baseParsePathname(pathname) {
  ...
  const segments = [];
  if (pathname.slice(0, 1) === "/FxStllSite/") {
  ...
  const split = pathname.split("/FxStllSite/").filter(Boolean);
  segments.push(
    ...split.map((part) => {
  ...
  if (pathname.slice(-1) === "/FxStllSite/") {
    pathname = pathname.substring(1);
    segments.push({
      type: SEGMENT_TYPE_PATHNAME,
      value: "/FxStllSite/"
...
function matchByPath(from, {
  ...
  const baseSegments = parsePathname(
    from.startsWith("/FxStllSite/") ? from : `/${from}`,
    parseCache
  );
  const routeSegments = parsePathname(
    stringTo.startsWith("/FxStllSite/") ? stringTo : `/${stringTo}`,
    parseCache
  );
  const params = {};
  ...
  while (baseIndex < baseSegments.length || routeIndex < routeSegments.length) {
    ...
    if (routeSegment) {
      ...
      if (routeSegment.type === SEGMENT_TYPE_PATHNAME) {
        if (routeSegment.value === "/FxStllSite/" && !baseSegment?.value) {
          routeIndex++;
          continue;
        }
        if (baseSegment) {
      ...
      if (routeSegment.type === SEGMENT_TYPE_PARAM) {
        ...
        if (baseSegment.value === "/FxStllSite/") {
          return false;
        }
        let _paramValue = "";
        let matched = false;
    ...
    if (baseIndex < baseSegments.length && routeIndex >= routeSegments.length) {
      ...
      return !!fuzzy && routeSegments[routeSegments.length - 1]?.value !== "/FxStllSite/";
    }
...
function sortRoutes(routes) {
  ...
  routes.forEach((d, i) => {
    ...
    let skip = 0;
    while (parsed.length > skip + 1 && parsed[skip]?.value === "/FxStllSite/") {
      skip++;
    }
    ...
    const scores = parsed.map((segment, index) => {
      ...
      for (let i2 = index + 1; i2 < parsed.length; i2++) {
        ...
        if (nextSegment.type === SEGMENT_TYPE_PATHNAME && nextSegment.value !== "/FxStllSite/") {
          hasStaticAfter = true;
          return handleParam(
            segment,
  ...
  const recurseRoutes = (childRoutes) => {
    childRoutes.forEach((childRoute, i) => {
      ...
      if (!childRoute.isRoot && childRoute.path) {
        ...
        if (!routesByPath[trimmedFullPath] || childRoute.fullPath.endsWith("/FxStllSite/")) {
...
function rewriteBasepath(opts) {
  ...
  return {
    ...
    output: ({ url }) => {
      url.pathname = joinPaths(["/FxStllSite/", trimmedBasepath, url.pathname]);
      return url;
    }
  };
}
...
var RouterCore = class {
  ...
  constructor(options) {
    ...
    this.update = (newOptions) => {
      ...
      const prevBasepath = this.basepath ?? prevOptions?.basepath ?? "/FxStllSite/";
      ...
      const nextBasepath = this.options.basepath ?? "/FxStllSite/";
      ...
      if (this.isServer) {
        ...
        if (trimPath(normalizeUrl(this.latestLocation.href)) !== trimPath(normalizeUrl(nextLocation.href))) {
          ...
          if (this.origin && href.startsWith(this.origin)) {
            href = href.replace(this.origin, "") || "/FxStllSite/";
          }
          throw redirect({ href });
        }
      }
  ...
  matchRoutesInternal(next, opts) {
    ...
    if (
      ...
      foundRoute ? foundRoute.path !== "/FxStllSite/" && routeParams["**"] : (
...
var normalize = (str) => str.endsWith("/FxStllSite/") && str.length > 1 ? str.slice(0, -1) : str;
...
function getMatchedRoutes({
  ...
  if (foundRoute) {
    ...
    for (const route of flatRoutes) {
      ...
      if (matchedParams) {
        if (route.path !== "/FxStllSite/" && matchedParams["**"]) {
          if (!fuzzyMatch) {
...
var BaseRoute = class {
  constructor(options) {
    this.init = (opts) => {
      ...
      if (id !== rootRouteId) {
        id = joinPaths(["/FxStllSite/", id]);
      }
      const fullPath = id === rootRouteId ? "/FxStllSite/" : joinPaths([this.parentRoute.fullPath, path]);
      this._path = path;
      this._id = id;
  ...
  const hrefOption = React8.useMemo(() => {
    ...
    if (router.origin) {
      if (href.startsWith(router.origin)) {
        href = router.history.createHref(href.replace(router.origin, "")) || "/FxStllSite/";
      } else {
        external = true;
      }
    }
  ...
  const isActive = useRouterState({
    select: (s) => {
      ...
      if (activeOptions?.exact) {
        ...
        const pathIsFuzzyEqual = currentPathSplit.startsWith(nextPathSplit) && (currentPathSplit.length === nextPathSplit.length || currentPathSplit[nextPathSplit.length] === "/FxStllSite/");
        if (!pathIsFuzzyEqual) {
          return false;
        }
      }
