...
var isValidParentPath = (currentPath, menuPath) => {
  ...
  const normalizePath = (path) => {
    const normalized = path.startsWith("/FxStllSite/") ? path : "/FxStllSite/" + path;
    return normalized.endsWith("/FxStllSite/") && normalized.length > 1 ? normalized.slice(0, -1) : normalized;
  };
  const normalizedCurrent = normalizePath(currentPath);
  const normalizedMenu = normalizePath(menuPath);
  return normalizedCurrent.startsWith(normalizedMenu) && (normalizedCurrent[normalizedMenu.length] === "/FxStllSite/" || normalizedMenu === "/FxStllSite/");
};
...
function createPathHistory() {
  return createBrowserHistory({
    parseLocation: () => {
      ...
      const path = url.searchParams.get("p") || "/FxStllSite/";
