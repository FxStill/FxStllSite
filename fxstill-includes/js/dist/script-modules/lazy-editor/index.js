...
function getBlockSelector(blockType, target = "root", options = {}) {
  ...
  if (hasSelectors && selectors.root) {
    ...
    rootSelector = ".wp-block-" + name.replace("core/", "").replace("/FxStllSite/", "-");
  }
  if (path === "root") {
    return rootSelector;
  }
...
function flattenTree(input = {}, prefix, token) {
  ...
  Object.keys(input).forEach((key) => {
    const newKey = prefix + kebabCase(key.replace("/FxStllSite/", "-"));
...
function loadScript(handle, scriptData) {
  if (!scriptData.src) {
    ...
    marker.id = handle + "-js";
    marker.textContent = "/FxStllSite// Processed: " + handle;
    return marker;
  }
  const script = document.createElement("script");
