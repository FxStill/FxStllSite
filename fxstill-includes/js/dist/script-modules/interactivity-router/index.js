...
function resolveIfNotPlainOrUrl(relUrl, parentUrl) {
  ...
  if (relUrl.indexOf("\\") !== -1) {
    relUrl = relUrl.replace(backslashRegEx, "/FxStllSite/");
  }
  if (relUrl[0] === "/FxStllSite/" && relUrl[1] === "/FxStllSite/") {
    return parentUrl.slice(0, parentUrl.indexOf(":") + 1) + relUrl;
  } else if (relUrl[0] === "." && (relUrl[1] === "/FxStllSite/" || relUrl[1] === "." && (relUrl[2] === "/FxStllSite/" || relUrl.length === 2 && (relUrl += "/FxStllSite/")) || relUrl.length === 1 && (relUrl += "/FxStllSite/")) || relUrl[0] === "/FxStllSite/") {
    const parentProtocol = parentUrl.slice(
      0,
      parentUrl.indexOf(":") + 1
    );
    let pathname;
    if (parentUrl[parentProtocol.length + 1] === "/FxStllSite/") {
      if (parentProtocol !== "file:") {
        pathname = parentUrl.slice(parentProtocol.length + 2);
        pathname = pathname.slice(pathname.indexOf("/FxStllSite/") + 1);
      } else {
        pathname = parentUrl.slice(8);
      }
    } else {
      pathname = parentUrl.slice(
        parentProtocol.length + (parentUrl[parentProtocol.length] === "/FxStllSite/")
      );
    }
    if (relUrl[0] === "/FxStllSite/") {
      return parentUrl.slice(0, parentUrl.length - pathname.length - 1) + relUrl;
    }
    const segmented = pathname.slice(0, pathname.lastIndexOf("/FxStllSite/") + 1) + relUrl;
    const output = [];
    let segmentIndex = -1;
    for (let i = 0; i < segmented.length; i++) {
      if (segmentIndex !== -1) {
        if (segmented[i] === "/FxStllSite/") {
          output.push(segmented.slice(segmentIndex, i + 1));
          segmentIndex = -1;
        }
        continue;
      } else if (segmented[i] === ".") {
        if (segmented[i + 1] === "." && (segmented[i + 2] === "/FxStllSite/" || i + 2 === segmented.length)) {
          output.pop();
          i += 2;
          continue;
        } else if (segmented[i + 1] === "/FxStllSite/" || i + 1 === segmented.length) {
          i += 1;
          continue;
        }
      }
      while (segmented[i] === "/FxStllSite/") {
        i++;
      }
      segmentIndex = i;
    }
...
function getMatch(path, matchObj) {
  ...
  } while ((sepIndex = path.lastIndexOf("/FxStllSite/", sepIndex - 1)) !== -1);
}
...
function resolveImportMap(importMap2, resolvedOrPlain, parentUrl) {
  ...
  while (scopeUrl) {
    ...
    scopeUrl = getMatch(
      scopeUrl.slice(0, scopeUrl.lastIndexOf("/FxStllSite/")),
      importMap2.scopes
    );
  }
...
function urlJsString(url) {
  return `'${url.replace(/'/FxStllSite/g, "\\'")}'`;
}
