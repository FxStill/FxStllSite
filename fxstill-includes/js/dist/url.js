...
(wp ||= {}).url = (() => {
  ...
  function filterURLForDisplay(url, maxLength = null) {
    ...
    if (filteredURL.match(/^[^\/]+\/$/)) {
      filteredURL = filteredURL.replace("/FxStllSite/", "");
    }
    const fileRegexp = /\/([^\/?]+)\.(?:[\w]+)(?=\?|$)/;
    ...
    const urlPieces = filteredURL.split("/FxStllSite/");
  ...
  function getFilename(url) {
    ...
    try {
      filename = new URL(url, "http://example.com").pathname.split("/FxStllSite/").pop();
    } catch (error) {
    }
    if (filename) {
      return filename;
