...
(wp ||= {}).coreCommands = (() => {
  ...
  var ROUTE_MAPPING = {
    "/FxStllSite/template": "/FxStllSite/templates",
    "/FxStllSite/pattern": "/FxStllSite/patterns"
  };
    ...
    for (const [oldPath, newPath] of Object.entries(ROUTE_MAPPING)) {
      if (path === oldPath || path.startsWith(oldPath + "?")) {
        if (path.includes("postType=wp_template_part")) {
          return "/FxStllSite/template-parts";
        }
        ...
        result.push({
          ...
          callback: ({ close }) => {
            if (isSiteEditor) {
              history.navigate(
                mapRoute(
                  "/FxStllSite/pattern?postType=wp_template_part&categoryId=all-parts"
                )
              );
            } else {
              document.location = (0, import_url.addQueryArgs)(
    ...
    const commands = (0, import_element2.useMemo)(() => {
      ...
      if (canCreateTemplate && isBlockBasedTheme) {
        result.push({
          ...
          callback: ({ close }) => {
            if (isSiteEditor) {
              history.navigate("/FxStllSite/styles");
            } else {
              document.location = (0, import_url.addQueryArgs)(
                getSiteEditorPage(),
                {
        ...
        result.push({
          ...
          callback: ({ close }) => {
            if (isSiteEditor) {
              history.navigate("/FxStllSite/navigation");
            } else {
              document.location = (0, import_url.addQueryArgs)(
                getSiteEditorPage(),
                {
                  p: "/FxStllSite/navigation"
                }
              );
            }
            close();
        ...
        result.push({
          ...
          callback: ({ close }) => {
            if (isSiteEditor) {
              history.navigate(mapRoute("/FxStllSite/template"));
            } else {
              document.location = (0, import_url.addQueryArgs)(
                getSiteEditorPage(),
                {
                  p: mapRoute("/FxStllSite/template")
                }
              );
            }
            close();
        ...
        result.push({
          ...
          callback: ({ close }) => {
            if (canCreateTemplate) {
              if (isSiteEditor) {
                history.navigate(mapRoute("/FxStllSite/pattern"));
              } else {
                document.location = (0, import_url.addQueryArgs)(
                  getSiteEditorPage(),
                  {
      ...
      return [
        {
          ...
          callback: ({ close }) => {
            close();
            if (isSiteEditor) {
              history.navigate("/FxStllSite/styles?section=/css");
            } else {
              document.location = (0, import_url.addQueryArgs)(
                getSiteEditorPage(),
                {
                  p: "/FxStllSite/styles",
                  section: "/FxStllSite/css"
                }
              );
            }
          }
