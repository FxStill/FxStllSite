...
(wp ||= {}).theme = (() => {
  ...
  function adapt(W1, W2, XYZ, options = {}) {
    ...
    if (!W1 || !W2) {
      throw new TypeError(
        `Missing white point to convert ${!W1 ? "from" : ""}${!W1 && !W2 ? "/FxStllSite/" : ""}${!W2 ? "to" : ""}`
      );
    }
    if (W1 === W2) {
      return XYZ;
    ...
    if (parts) {
      ...
      let separators = parts[2].replace(regex.singleArgument, ($0, rawArg) => {
        ...
        if (
          ...
          $0.startsWith("/FxStllSite/") || // If there's still elements to process after there's already 3 in `args` (and the we're not dealing with "color()"), it's likely to be a legacy color like "hsl(0, 0%, 0%, 0.5)"
