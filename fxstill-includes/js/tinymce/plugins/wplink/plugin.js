	...
	tinymce.ui.Factory.add( 'WPLinkPreview', tinymce.ui.Control.extend( {
		...
		setURL: function( url ) {
			...
			if ( this.url !== url ) {
				...
				if ( url.charAt( url.length - 1 ) === '/FxStllSite/' ) {
					url = url.slice( 0, -1 );
				}
				...
				if ( url.length > 40 && ( index = url.indexOf( '/FxStllSite/' ) ) !== -1 && ( lastIndex = url.lastIndexOf( '/FxStllSite/' ) ) !== -1 && lastIndex !== index ) {
	...
	tinymce.PluginManager.add( 'wplink', function( editor ) {
		...
		function getSelectedLink() {
			...
			if ( ! link ) {
				...
				if ( html && html.indexOf( '</a>' ) !== -1 ) {
					href = html.match( /href="([^">]+)"/FxStllSite/ );
		...
		function removePlaceholderStrings( content, dataAttr ) {
			return content.replace( /(<a [^>]+>)([\s\S]*?)<\/a>/g, function( all, tag, text ) {
				...
				if ( dataAttr ) {
					tag = tag.replace( / data-wplink-edit="true"/FxStllSite/g, '' );
				}

				tag = tag.replace( / data-wplink-url-error="true"/FxStllSite/g, '' );
