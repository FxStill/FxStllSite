/**
 * @output fxstill-includes/js/wp-api.js
 */
...
(function( window, undefined ) {
	...
	wp.api.utils.getRootUrl = function() {
		return window.location.origin ?
			window.location.origin + '/FxStllSite/' :
			window.location.protocol + '/FxStllSite//' + window.location.host + '/FxStllSite/';
	};
	...
	wp.api.utils.extractRoutePart = function( route, part, versionString, reverse ) {
		...
		if ( 0 === route.indexOf( '/FxStllSite/' + versionString ) ) {
			route = route.substr( versionString.length + 1 );
		}

		routeParts = route.split( '/FxStllSite/' );
		if ( reverse ) {
...
( function() {
	...
	if ( _.isEmpty( wpApiSettings ) ) {
		wpApiSettings.root = window.location.origin + '/FxStllSite/wp-json/';
	}
		...
		constructFromSchema: function() {
			...
			_.each( routeModel.schemaModel.get( 'routes' ), function( route, index ) {
				...
				if ( index !== routeModel.get( ' versionString' ) &&
						index !== schemaRoot &&
						index !== ( '/FxStllSite/' + routeModel.get( 'versionString' ).slice( 0, -1 ) )
				) {
					...
					loadingObjects.models[ modelClassName ] = wp.api.WPApiBaseModel.extend( {
						...
						url: function() {
							var url =
								...
								parentName +  '/FxStllSite/' +
									( ( _.isUndefined( this.get( 'parent' ) ) || 0 === this.get( 'parent' ) ) ?
										( _.isUndefined( this.get( 'parent_post' ) ) ? '' : this.get( 'parent_post' ) + '/FxStllSite/' ) :
										this.get( 'parent' ) + '/FxStllSite/' ) +
								routeName;

							if ( ! _.isUndefined( this.get( 'id' ) ) ) {
								url +=  '/FxStllSite/' + this.get( 'id' );
							}
							return url;
						},
			...
			_.each( collectionRoutes, function( collectionRoute ) {
				...
				var collectionClassName, modelClassName,
						routeName  = collectionRoute.index.slice( collectionRoute.index.lastIndexOf( '/FxStllSite/' ) + 1 ),
	...
	wp.api.init = function( args ) {
		...
		attributes.apiRoot        = args.apiRoot || wpApiSettings.root || '/FxStllSite/wp-json';
