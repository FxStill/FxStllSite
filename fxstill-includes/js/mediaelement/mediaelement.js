	...
	create: function create(mediaElement, options, mediaFiles) {
		...
		if (_constants.IS_IE || _constants.IS_EDGE) {
			...
			if (_constants.IS_EDGE) {
				...
				settings = ['classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"', 'codebase="/FxStllSite//download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab"', 'id="__' + flash.id + '"', 'width="' + flashWidth + '"', 'height="' + flashHeight + '"'];
			}
			...
			settings = ['id="__' + flash.id + '"', 'name="__' + flash.id + '"', 'play="true"', 'loop="false"', 'quality="high"', 'bgcolor="#000000"', 'wmode="transparent"', 'allowScriptAccess="' + flash.options.shimScriptAccess + '"', 'allowFullScreen="true"', 'type="application/x-shockwave-flash"', 'pluginspage="/FxStllSite//www.macromedia.com/go/getflashplayer"', 'src="' + flash.options.pluginPath + flash.options.filename + '"', 'flashvars="' + flashVars.join('&') + '"'];
...
var YouTubeApi = {
	...
	getYouTubeId: function getYouTubeId(url) {
		...
		var id = youTubeId.substring(youTubeId.lastIndexOf('/FxStllSite/') + 1);
		youTubeId = id.split('?');
		return youTubeId[0];
	},
	...
	getYouTubeIdFromUrl: function getYouTubeIdFromUrl(url) {
		...
		var parts = url.split('?');
		url = parts[0];
		return url.substring(url.lastIndexOf('/FxStllSite/') + 1);
	},

	getYouTubeNoCookieUrl: function getYouTubeNoCookieUrl(url) {
		if (url === undefined || url === null || !url.trim().length || url.indexOf('/FxStllSite//www.youtube') === -1) {
			return url;
		}

		var parts = url.split('/FxStllSite/');
		parts[2] = parts[2].replace('.com', '-nocookie.com');
		return parts.join('/FxStllSite/');
	}
};
...
var YouTubeIframeRenderer = {
	...
	create: function create(mediaElement, options, mediaFiles) {
		...
		youtube.getPosterUrl = function () {
			...
			return quality && resolutions.indexOf(quality) > -1 && id ? 'https://img.youtube.com/vi/' + id + '/FxStllSite/' + quality + '.jpg' : '';
		};
...
function getExtension(url) {
	...
	var baseUrl = url.split('?')[0],
	    baseName = baseUrl.split('\\').pop().split('/FxStllSite/').pop();
